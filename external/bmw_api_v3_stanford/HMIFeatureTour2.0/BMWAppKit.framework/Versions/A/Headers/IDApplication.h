...
@class IDVehicleInfo;
@class IDView;
@class IDImageDataBundle;
@class IDImageData;
@class IDTextBundle;

typedef enum {
    IDApplicationTypeMultimedia,
    IDApplicationTypeRadio,
    IDApplicationTypeAddressBook,
    IDApplicationTypeNavigation,
    IDApplicationTypeOnlineServices,
    IDApplicationTypePhone,
    IDApplicationTypeSettings,
    IDApplicationTypeVehicleInfo
} IDApplicationType;
...
/*!
 ...
 @abstract The <keyword>IDApplicationDataSource<keyword> protocol provides an <reftype>IDApplication</reftype> object with the information it needs.
 */
...
/*!
 ...
 @discussion The resulting array can contain multiple text databases to be used. The application determines which text databases it needs to provide depending on the properties brand and hmiType of the currently connected vehicle (@see IDVehicleInfo). If multiple databases contain a text resource with the same identifier, the text resource in the database with the highest index in the returned array will be used.
 */
...
/*!
 ...
 @discussion The resulting array can contain multiple image databases to be used. The application determines which image databases it needs to provide depending on the properties brand and hmiType of the currently connected vehicle (@see IDVehicleInfo). If multiple databases contain an image with the same identifier, the image in the database with the highest index in the returned array will be used.
 */
...
/*!
 ...
 @abstract The IDApplicationDelegate protocol provides an <ref>IDApplication</ref> object with the information it needs.
 */
...
/*!
 ...
 @discussion When this message is received, the application should restore its main hmi state. Restoring the main HMI state is also known as LUM (Last User Mode). Be aware that LUM is not supported in all countries. If LUM is not supported this method will not be called. This method might get called before or after the application did start (@see applicationDidStart:).
...
/*!
 ...
 @param application
    the application instance that did connect to the vehicle (@see IDApplication)
 @param vehicleInfo
    some information about the connected vehicle (@see IDVehicleInfo)
 */
...
/*!
 ...
 @param application
    the application instance that did start (@see IDApplication)
 */
...
/*!
 ...
 @param application
    the application instance that did fail to start (@see IDApplication)
 @param error
...
/*!
 ...
 @param application
    the application instance that did stop (@see IDApplication)
 */
...
/*!
 @method registerApplicationWithManifest:
 @abstract Registers an application for the AppSwitcher without setting the IDApplicationDataSource
 @discussion This method registers a model for app switching purposes with the information provided in the manifest. This way, it is not necessary to instantiate an IDApplication *and* set the corresponding IDApplicationDataSource in order to register a model to the AppSwichter.
 @param dictionary
    NSDictionary containing a manifest, e.g. via [NSDictionary dictionaryWithContentsOfFile:]
 @return NO if registration failed, otherwise YES
 */
+ (BOOL)registerApplicationWithManifest:(NSDictionary *)dictionary;
...
@property (retain, readonly) IDImageDataBundle *imageBundle;
@property (nonatomic, retain) IDTextBundle *textBundle;
@property (nonatomic, retain) IDImageData *entryIcon;
@property (nonatomic, assign) IDApplicationType type;
